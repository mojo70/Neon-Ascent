    ...
    fun provideDatabase(
        ...
        if (dbFile.exists()) {
            ...
            try {
                ...
                Log.e("DatabaseModule", "NeonAscentDatabase verification failed. Preserving file in place.", e)
            } finally {
                try { db?.close() } catch (_: Throwable) {}
            }

            if (!openSuccessful) {
                val errorMsg = "NeonAscentDatabase verification failed for $dbName. Leaving database file intact in place."
